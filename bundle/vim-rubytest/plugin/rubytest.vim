...
function s:FindRoot()
	return strpart(getcwd(), 0, strridx(getcwd(), 'rails')) . 'rails'
endfunction
...
function s:RunSpec()
  ...
  let cmd = "cd " . s:FindRoot() . ";" . cmd
  let file = s:EscapeBackSlash(@%)
  if strridx(file, 'rails') == -1
